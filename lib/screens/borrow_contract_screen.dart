      ...
      body: SafeArea(
        child: Column(
          children: [
            const CustomHeader(
              showBackButton: true,
              title: "Borrow Contracts",
            ),
            Expanded(
              child: FutureBuilder<List<BorrowContract>>(
                ...
                builder: (context, snapshot) {
                  ...
                  contracts
                      .sort((a, b) => b.contractId.compareTo(a.contractId));

                  return FutureBuilder<List<DonateItem>>(
                    future: Future.wait(
                      contracts
                          .map((c) => ApiService.fetchDonateItemById(c.itemId)),
                    ),
                    builder: (context, itemSnap) {
                      if (itemSnap.connectionState == ConnectionState.waiting) {
                        ...
                        return const Center(
                            child: Text("Error loading item data"));
                      } else {
